...
namespace MCDawn
{
    ...
    public class Level
    {
        ...
        public delegate void OnLevelLoadEventHandler(string givenName, byte phys);
		public static event OnLevelLoadEventHandler OnLevelLoadEvent = null;

        public delegate void OnLevelLoadedEventHandler(string givenName, byte phys);
        public static event OnLevelLoadedEventHandler OnLevelLoadedEvent = null;

        public delegate void OnLevelSaveEventHandler(Level l, bool Override);
        public static event OnLevelSaveEventHandler OnLevelSaveEvent = null;

        public delegate void OnSaveEventHandler(bool Override);
        public event OnSaveEventHandler OnSaveEvent = null;

        public delegate void OnLevelUnloadEventHandler(Level l, bool stealth);
        public static event OnLevelUnloadEventHandler OnLevelUnloadEvent = null;

        public delegate void OnLevelPhysicsUpdateEventHandler(Level l, ushort x, ushort y, ushort z, byte time, string extraInfo);
        public static event OnLevelPhysicsUpdateEventHandler OnLevelPhysicsUpdateEvent = null;

        public delegate void OnPhysicsUpdateEventHandler(ushort x, ushort y, ushort z, byte time, string extraInfo);
        public event OnPhysicsUpdateEventHandler OnPhysicsUpdateEvent = null;

        public bool noSave = false, noUnload = false, noPhysicsUpdate = false;
        public static bool noLoad = false;
        ...
        public bool spleefstarted = false;
        public List<Player> spleefAlive { get { return getSpleefAlive(); } }

        public System.Timers.Timer countDown = new System.Timers.Timer(1000);

        //Infection
        public Infection infection = new Infection();
        public bool zombiegame = false;
        public string queuename = "";
        public List<Player> zombies { get { return getInfected(); } }
        public List<Player> humans { get { return getHumans(); } }
        public List<Player> referees { get { return getReferees(); } }

        //GunGame
        public GunGame gunGame = new GunGame();
        public bool gunGameActive = false;

        //PushBall
        public PushBall pushBall = new PushBall();
        public bool pushBallStarted = false;
        ...
        public struct Reflection
        {
            ...
            public List<ReflectionPos> reflectionPositions; 
        }
        public enum ReflectionType { Line, Cross, Platform, Cross3D }
        public struct ReflectionPos { public ushort x, y, z; }
        
        public struct ReflectionLinePos { public byte type; public ushort x, y, z; }
        public Dictionary<string, List<Level.ReflectionLinePos>> reflectionLines = new Dictionary<string, List<Level.ReflectionLinePos>>();

        public void ShowReflectionsLines(Player p)
        {
            for (int i = 0; i < reflectionLines.Keys.Count; i++)
            {
                List<ReflectionLinePos> list = new List<List<ReflectionLinePos>>(reflectionLines.Values)[i];
                for (int ii = 0; ii < list.Count; ii++)
                    p.SendBlockchange(list[ii].x, list[ii].y, list[ii].z, Block.blackrock);
            }
        }

        public void HideReflectionLines(Player p)
        {
            for (int i = 0; i < reflectionLines.Keys.Count; i++)
            {
                List<ReflectionLinePos> list = new List<List<ReflectionLinePos>>(reflectionLines.Values)[i];
                for (int ii = 0; ii < list.Count; ii++)
                    p.SendBlockchange(list[ii].x, list[ii].y, list[ii].z, list[ii].type);
            }
        }
        ...
        public Level(string n, ushort x, ushort y, ushort z, string type, byte b = 20)
        {
            ...
            switch (type)
            {
                ...
                case "sphere":
                case "dome":
                    Server.MapGen.CreateSphere(this, b);
                    break;
                case "nether":
                case "hell":
        ...
        public bool Unload(bool stealth = false)
        {
            if (Level.OnLevelUnloadEvent != null) Level.OnLevelUnloadEvent(this, stealth);

            if (noUnload) return false;
        ...
        public static Level Find(string levelName/*, bool exp = false*/)
        {
            //if (levelName[0] == '!') { levelName.Remove(0, 1); exp = true; }
        ...
        public void Blockchange(Player p, ushort x, ushort y, ushort z, byte type, bool addacation)
        {
    ...
    retry:  try
            {
                ...
                if ((p.group.Permission < LevelPermission.Admin || p.ZoneCheck || p.zoneDel) && !Block.AllowBreak(b))
                {
                        ...
                        foreach (Zone Zn in ZoneList)
                        {
                            if (Zn.smallX <= x && x <= Zn.bigX && Zn.smallY <= y && y <= Zn.bigY && Zn.smallZ <= z && z <= Zn.bigZ)
                            {
                                ...
                                if (p.zoneDel)
                                {
                                    ...
                                    foundDel = true;
                                    if (p.level.name == Server.HomePrefix + p.name.ToLower()) { Command.all.Find("zoneall").Use(p, Server.HomePrefix + p.name.ToLower()); }
                                }
                                else
                                {
                    ...
                    if (p.zoneDel)
                    {
                        ...
                        {
                            foreach (Zone Zn in toDel) ZoneList.Remove(Zn);
                            p.SendBlockchange(x, y, z, b);
                            if (name.ToLower() == Server.HomePrefix + p.name.ToLower()) Command.all.Find("zoneall").Use(p, p.name);
                        }
                        p.zoneDel = false;
                        return;
                    }
                ...
                errorLocation = "Reflections";        //Used when "reflections" are active (live mirroring).
                if (reflections.ContainsKey(p.name.ToLower()))
                {
                    Reflection r = new Reflection();
                    for (int i = 0; i < reflections.Keys.Count; i++)
                        if (p.name.ToLower() == reflections.Keys.ToList<string>()[i])
                            r = reflections.Values.ToList<Reflection>()[i];
                    switch (r.reflectionType)
                    {
                        case ReflectionType.Line:
                            List<ReflectionLinePos> lrlp = new List<ReflectionLinePos>();
                            for (int i = 0; i < reflectionLines.Keys.Count; i++)
                                if (new List<string>(reflectionLines.Keys)[i] == p.name.ToLower())
                                    lrlp = new List<List<ReflectionLinePos>>(reflectionLines.Values)[i];
                            double rx = Math.Max(lrlp[0].x, lrlp[lrlp.Count].x) - Math.Min(lrlp[0].x, lrlp[lrlp.Count].x);
                            double rz = Math.Max(lrlp[0].z, lrlp[lrlp.Count].z) - Math.Min(lrlp[0].z, lrlp[lrlp.Count].z);
                            // slope is (dz / dx) (rise over run)
                            // perpendicular line is negative reciprocal, which would mean new ratio is -(dx / dz)
                            // hur dur magic here
                            ushort xx, zz;
                            ...
                            Blockchange(p, x, y, z, type);
                            break;
                        case ReflectionType.Cross:
                            break;
                        case ReflectionType.Platform:
                            break;
                        case ReflectionType.Cross3D:
                            break;
                    }
                }
        ...
        {
            if (Level.OnLevelSaveEvent != null) Level.OnLevelSaveEvent(this, Override);
            if (this.OnSaveEvent != null) this.OnSaveEvent(Override);
            ...
            try
            {
                ...
                if (changed == true || !File.Exists(path) || Override)
                {
                    ...
                    SW.WriteLine("Locked = " + locked.ToString());
                    SW.WriteLine("Allow Guns = " + allowguns.ToString());
                    SW.WriteLine("SpeedHack Rank = " + speedHackRank.name);
                    SW.Flush();
                    SW.Close();
                                ...
                                if (line[0] != '#')
                                {
                                    ...
                                    switch (line.Substring(0, line.IndexOf(" = ")).ToLower())
                                    {
                                        ...
                                        case "pervisit":
                                            ...
                                            break;
                                        case "locked": level.locked = bool.Parse(value); break;
                                        case "allow guns": level.allowguns = bool.Parse(value); break;
                                        case "speedhack rank": 
                                            try { level.speedHackRank = Group.Find(value); }
                                            catch { level.speedHackRank = Group.findPerm(LevelPermission.Banned); } 
                                            break;
                                    }
                                }
                            }
                ...
                if (physics == 5)
                {
                    ...
                    ListCheck.ForEach(delegate(Check C)
                    {
                        try
                        {
                            ...
                            if (Level.OnLevelPhysicsUpdateEvent != null) Level.OnLevelPhysicsUpdateEvent(this, x, y, z, C.time, C.extraInfo);
                            if (this.OnPhysicsUpdateEvent != null) this.OnPhysicsUpdateEvent(x, y, z, C.time, C.extraInfo);

                            if (noPhysicsUpdate) return;
        ...
        {
            List<PlayerBot> foundPlayers = new List<PlayerBot>();
            foreach (PlayerBot p in PlayerBot.playerbots)
            {
                if (p.level == this) foundPlayers.Add(p);
            }
            return foundPlayers;
        }
        public List<Player> getSpleefAlive()
        {
            List<Player> foundPlayers = new List<Player>();
            foreach (Player p in Player.players)
            {
                if (p.level == this && p.spleefAlive) foundPlayers.Add(p);
            }
            return foundPlayers;
        }
        public List<Player> getInfected()
        {
            ...
            foreach (Player p in Player.players)
            {
                if (p.level == this && p.infected && !p.referee) foundPlayers.Add(p);
            }
            return foundPlayers;
        }
        ...
        public List<Player> getReferees()
        {
            ...
            foreach (Player p in Player.players)
            {
                if (p.level == this && p.referee) foundPlayers.Add(p);
            }
            return foundPlayers;
        }
    }
