...
namespace MCDawn.Gui
{
    public partial class PropertyWindow : Form
    {
        ...
        private void PropertyWindow_Load(object sender, EventArgs e) {
            ...
            cmbGlobalColour.Items.AddRange(colors);
            cmbColor.Items.AddRange(colors);

            string opchatperm = "";
            string adminchatperm = "";
            string canjoinmaint = "";
            string adminsecurityrank = "";
            string homeperm = "";
            foreach (Group grp in Group.GroupList)
            {
                ...
                cmbMaintJoin.Items.Add(grp.name);
                if (grp.Permission == Server.canjoinmaint)
                {
                    canjoinmaint = grp.name;
                }
                cmbAdminSecurityRank.Items.Add(grp.name);
                if (grp.Permission == Server.adminsecurityrank)
                {
                    adminsecurityrank = grp.name;
                }
                cmbHomePerm.Items.Add(grp.name);
            ...
            cmbMaintJoin.SelectedIndex = (canjoinmaint != "") ? cmbMaintJoin.Items.IndexOf(canjoinmaint) : 1;
            cmbAdminSecurityRank.SelectedIndex = (adminsecurityrank != "") ? cmbMaintJoin.Items.IndexOf(adminsecurityrank) : 1;
            cmbHomePerm.SelectedIndex = (homeperm != "") ? cmbHomePerm.Items.IndexOf(homeperm) : 1;

            if (Server.tpToHigher == true) { chkTpToHigher.Checked = true; }
            else { chkTpToHigher.Checked = false; }

            txtMySQLPass.PasswordChar = '*';
            txtIrcPass.PasswordChar = '*';
            txtGlobalPass.PasswordChar = '*';
            //txtMySQLPass.UseSystemPasswordChar = true;

            lblPortCheckResult.Text = "Not Started";
            ...
            {
                Server.s.Log("Failed to load properties!");
            }
        }
                    ...
                    if (line != "" && line[0] != '#') {
                        ...
                        switch (key.ToLower())
                        {
                            case "server-name":
                                ...
                                else txtName.Text = "[MCDawn] Default";
                                break;
                            case "description":
                                if (ValidString(value, "![]:.,{}~-+()?_/\\ ")) txtDescription.Text = value;
                                else txtDescription.Text = "MCDawn Server";
                                break;
                            case "flags":
                                if (ValidString(value, "![]:.,{}~-+()?_/\\ ")) txtFlags.Text = value;
                                else txtFlags.Text = "MCDawn";
                                break;
                            case "motd":
                                if (ValidString(value, "=![]&:.,{}~-+()?_/\\' ")) txtMOTD.Text = value;
                                else txtMOTD.Text = "Welcome! +hax";
                                break;
                            case "port":
                            ...
                            case "verify-names":
                                ...
                                break;
                            case "allowproxy":
                                chkAllowProxy.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "public":
                            ...
                            case "max-players":
                                ...
                                break;
                            case "max-guests":
                                try { txtMaxGuests.Text = value; }
                                catch
                                {
                                    Server.s.Log("max-players invalid! setting to default.");
                                    txtMaxGuests.Text = "50";
                                }
                                break;
                            case "max-maps":
                                try {
                            ...
                            case "opchat-perm":
                                try {
                                    if (cmbOpChat.Items.IndexOf(value.ToLower()) != -1)
                                        cmbOpChat.SelectedIndex = cmbOpChat.Items.IndexOf(value.ToLower());
                                } catch { cmbOpChat.SelectedIndex = 1; }
                                break;

                            case "adminchat-perm":
                                try {
                                    if (cmbAdminChat.Items.IndexOf(value.ToLower()) != -1)
                                        cmbAdminChat.SelectedIndex = cmbAdminChat.Items.IndexOf(value.ToLower());
                                } catch { cmbAdminChat.SelectedIndex = 1; }
                                break;
                            ...
                            case "cheap-message-given":
                                ...
                                break;
                            case "uncheapmessage":
                                chkUnCheap.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            ...
                            case "host-state":
                                ...
                                break;
                            case "usemysql":
                                chkUseMySQL.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "host":
                                try
                                {
                                    txtMySQLHost.Text = value;
                                }
                                catch
                                {
                                    txtMySQLHost.Text = "127.0.0.1";
                                }
                                break;
                            case "sqlport":
                                try
                                ...
                                break;
                            case "username":
                                if (value != "") txtMySQLUser.Text = value;
                                else txtMySQLUser.Text = "root";
                                break;
                            case "password":
                                if (value != "") txtMySQLPass.Text = value;
                                else txtMySQLPass.Text = "password";
                                break;
                            case "databasename":
                                if (value != "") txtMySQLDatabase.Text = value;
                                else txtMySQLDatabase.Text = "MCDawnDB";
                                break;
                            case "antispam":
                                chkAntiSpam.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "antispamop":
                                chkAntiSpamOp.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "antispamstyle":
                                if (value != "") { cmbAntiSpamConsequence.Text = value; }
                                else { cmbAntiSpamConsequence.Text = "Kick"; }
                                break;
                            case "msgsrequired":
                                if (value != "") { txtMsgsRequired.Text = value; }
                                else { txtMsgsRequired.Text = "4"; }
                                break;
                            case "anticaps":
                                chkAntiCaps.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "anticapsop":
                                chkAntiCapsOp.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "anticapsstyle":
                                if (value != "") { cmbAntiCapsConsequence.Text = value; }
                                else { cmbAntiCapsConsequence.Text = "Kick"; }
                                break;
                            case "capsrequired":
                                if (value != "") { txtCapsRequired.Text = value; }
                                else { txtCapsRequired.Text = "10"; }
                                break;
                            case "useglobal":
                                chkGlobalChat.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "globalnick":
                                txtGlobalNick.Text = validNick(value);
                                break;
                            case "global-identify":
                                chkGlobalIdentify.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "global-password":
                                txtGlobalPass.Text = value; 
                                break;
                            case "adminsecurity":
                                chkAdminSecurity.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "adminsecurityrank":
                                try {
                                    if (cmbAdminSecurityRank.Items.IndexOf(value.ToLower()) != -1)
                                        cmbAdminSecurityRank.SelectedIndex = cmbAdminSecurityRank.Items.IndexOf(value.ToLower());
                                }
                                catch { cmbAdminSecurityRank.SelectedIndex = 1; }
                                break;
                            case "join-on-maintenence":
                                try {
                                    if (cmbMaintJoin.Items.IndexOf(value.ToLower()) != -1)
                                        cmbMaintJoin.SelectedIndex = cmbMaintJoin.Items.IndexOf(value.ToLower());
                                }
                                catch { cmbMaintJoin.SelectedIndex = 1; }
                                break;
                            case "global-color":
                                color = c.Parse(value);
                                if (color == "")
                                ...
                                cmbGlobalColour.SelectedIndex = cmbGlobalColour.Items.IndexOf(c.Name(value)); break;
                            case "adminsjoinsilent":
                                chkAdminsJoinSilent.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "adminsjoinhidden":
                                chkAdminsJoinHidden.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "countryonjoin":
                                chkUseMaxMind.Checked = (value.ToLower() == "true") ? true : false;
                                break;
                            case "agreetorules":
                            ...
                            case "home-perm":
                                try
                                {
                                    if (cmbHomePerm.Items.IndexOf(value.ToLower()) != -1)
                                        cmbHomePerm.SelectedIndex = cmbHomePerm.Items.IndexOf(value.ToLower());
                                }
                                catch { cmbHomePerm.SelectedIndex = 1; }
                                break;
                            case "homeprefix":
                                txtHomePrefix.Text = value;
                                break;
                            case "home-x":
                                if (value != "") { cmbHomeX.Text = value; }
                                else { txtMsgsRequired.Text = "128"; }
                                break;
                            case "home-y":
                                if (value != "") { cmbHomeY.Text = value; }
                                else { txtMsgsRequired.Text = "128"; }
                                break;
                            case "home-z":
                                if (value != "") { cmbHomeZ.Text = value; }
                                else { txtMsgsRequired.Text = "128"; }
                                break;
                            case "show-attempted-logins":
                                try { chkShowAttemptedLogins.Checked = bool.Parse(value); }
                                catch { Server.s.Log("Invalid " + key + ". Using default."); break; }
                                break;
                            case "profanityfilter":
                                try { chkProfanityFilter.Checked = bool.Parse(value); }
                                catch { Server.s.Log("Invalid " + key + ". Using default."); break; }
                                break;
                            case "warnplayer":
                                try { chkSwearWarnPlayer.Checked = bool.Parse(value); }
                                catch { Server.s.Log("Invalid " + key + ". Using default."); break; }
                                break;
                            case "swearwordsrequired":
                                txtSwearWordsRequired.Text = value;
                                break;
                            case "apply-to-op":
                                try { chkProfanityFilterOp.Checked = bool.Parse(value); }
                                catch { Server.s.Log("Invalid " + key + ". Using default."); break; }
                                break;
                            case "profanityfilterstyle":
                                cmbProfanityFilterStyle.Text = value;
                                break;
                            case "useantigrief":
                                try { chkUseAntiGrief.Checked = bool.Parse(value); }
                                catch { Server.s.Log("Invalid " + key + ". Using default."); break; }
                                break;
                            case "allow-ignore-ops":
                                try { chkAllowIgnoreOps.Checked = bool.Parse(value); }
                                catch { Server.s.Log("Invalid " + key + ". Using default."); break; }
                                break;
                            case "use-timerank":
                                try { chkUseTimeRank.Checked = bool.Parse(value); }
                                catch { Server.s.Log("Invalid " + key + ". Using default."); break; }
                                break;
                            case "timerank-cmd":
                                txtTimeRankCmd.Text = value;
                                break;
                            case "use-wom":
                                try { chkUseWOM.Checked = bool.Parse(value); }
                                catch { Server.s.Log("Invalid " + key + ". Using default."); break; }
                                break;
                            case "wom-text":
                                try { chkWomText.Checked = bool.Parse(value); }
                                catch { Server.s.Log("Invalid " + key + ". Using default."); break; }
                                break;
                            case "use-discourager":
                                try { chkUseDiscourager.Checked = bool.Parse(value); }
                                catch { Server.s.Log("Invalid " + key + ". Using default."); break; }
                                break;
                            case "throttle":
        ...
        public void Save(string givenPath) {
            try {
                ...
                if (givenPath.IndexOf("server") != -1) {
                    ...
                    w.WriteLine("#   description\t=\tYour server's description in the WOM Server list.");
                    w.WriteLine("#   flags\t=\tYour server's flags in the WOM Server list.");
                    ...
                    w.WriteLine("description = " + txtDescription.Text);
                    w.WriteLine("flags = " + txtFlags.Text);
                    w.WriteLine("motd = " + txtMOTD.Text);
                    w.WriteLine("port = " + txtPort.Text);
                    w.WriteLine("use-upnp = " + chkUPnP.Checked.ToString().ToLower());
                    ...
                    w.WriteLine("use-wom = " + chkUseWOM.Checked.ToString().ToLower());
                    w.WriteLine();
                    w.WriteLine("# irc bot options");
                    w.WriteLine("irc = " + chkIRC.Checked.ToString());
                    w.WriteLine("irc-nick = " + validNick(txtNick.Text));
                    ...
                    w.WriteLine("UseMySQL = " + chkUseMySQL.Checked.ToString().ToLower());
                    w.WriteLine("Host = " + txtMySQLHost.Text);
                    w.WriteLine("SQLPort = " + txtMySQLPort.Text);
                    w.WriteLine("Username = " + txtMySQLUser.Text);
                    w.WriteLine("Password = " + txtMySQLPass.Text);
                    w.WriteLine("DatabaseName = " + txtMySQLDatabase.Text);
                    ...
                    w.WriteLine("uncheapmessage = " + chkUnCheap.Checked.ToString().ToLower());
                    ...
                    w.WriteLine();
                    w.WriteLine("#Homes");
                    w.WriteLine("home-perm = " + ((sbyte)Group.GroupList.Find(grp => grp.name == cmbHomePerm.Items[cmbHomePerm.SelectedIndex].ToString()).Permission).ToString());
                    w.WriteLine("homeprefix = " + txtHomePrefix.Text);
                    w.WriteLine("home-x = " + cmbHomeX.Text);
                    w.WriteLine("home-y = " + cmbHomeY.Text);
                    w.WriteLine("home-z = " + cmbHomeZ.Text);
                    w.WriteLine();
                    w.WriteLine("#Time Ranks");
                    w.WriteLine("use-timerank = " + chkUseTimeRank.Checked.ToString().ToLower());
                    w.WriteLine("timerank-cmd = " + txtTimeRankCmd.Text.ToLower());
                    w.WriteLine();
                    w.WriteLine("#Profanity Filter");
                    w.WriteLine("profanityfilter = " + chkProfanityFilter.Checked.ToString().ToLower());
                    w.WriteLine("warnplayer = " + chkSwearWarnPlayer.Checked.ToString().ToLower());
                    w.WriteLine("swearwordsrequired = " + txtSwearWordsRequired.Text);
                    w.WriteLine("apply-to-op = " + chkProfanityFilterOp.Checked.ToString().ToLower());
                    w.WriteLine("profanityfilterstyle = " + cmbProfanityFilterStyle.Text);
                    w.WriteLine();
                    w.WriteLine("#More Features");
                    w.WriteLine("antispam = " + chkAntiSpam.Checked.ToString().ToLower());
                    w.WriteLine("antispamop = " + chkAntiSpamOp.Checked.ToString().ToLower());
                    w.WriteLine("antispamstyle = " + cmbAntiSpamConsequence.Text.ToLower());
                    w.WriteLine("msgsrequired = " + txtMsgsRequired.Text);
                    w.WriteLine("anticaps = " + chkAntiCaps.Checked.ToString().ToLower());
                    w.WriteLine("anticapsop = " + chkAntiCapsOp.Checked.ToString().ToLower());
                    w.WriteLine("anticapsstyle = " + cmbAntiCapsConsequence.Text.ToLower());
                    w.WriteLine("capsrequired = " + txtCapsRequired.Text);
                    w.WriteLine("useglobal = " + chkGlobalChat.Checked.ToString().ToLower());
                    w.WriteLine("globalnick = " + validNick(txtGlobalNick.Text));
                    w.WriteLine("global-identify = " + chkGlobalIdentify.Checked.ToString().ToLower());
                    w.WriteLine("global-password = " + txtGlobalPass.Text);
                    w.WriteLine("adminsecurity = " + chkAdminSecurity.Checked.ToString().ToLower());
                    w.WriteLine("adminsecurityrank = " + ((sbyte)Group.GroupList.Find(grp => grp.name == cmbAdminSecurityRank.Items[cmbAdminSecurityRank.SelectedIndex].ToString()).Permission).ToString());
                    w.WriteLine("join-on-maintenence = " + ((sbyte)Group.GroupList.Find(grp => grp.name == cmbMaintJoin.Items[cmbMaintJoin.SelectedIndex].ToString()).Permission).ToString());
                    w.WriteLine("global-color = " + cmbGlobalColour.Items[cmbGlobalColour.SelectedIndex].ToString());
                    w.WriteLine("adminsjoinsilent = " + chkAdminsJoinSilent.Checked.ToString().ToLower());
                    w.WriteLine("adminsjoinhidden = " + chkAdminsJoinHidden.Checked.ToString().ToLower());
                    w.WriteLine("countryonjoin = " + chkUseMaxMind.Checked.ToString().ToLower());
                    w.WriteLine("allowproxy = " + chkAllowProxy.Checked.ToString().ToLower());
                    w.WriteLine("agreetorules = " + chkAgreeToRules.Checked.ToString().ToLower());
                    w.WriteLine("agreepass = " + Server.agreePass);
                    w.WriteLine("consolesound = " + Server.consoleSound);
                    w.WriteLine("show-attempted-logins = " + chkShowAttemptedLogins.Checked.ToString().ToLower());
                    w.WriteLine("useantigrief = " + chkUseAntiGrief.Checked.ToString().ToLower());
        ...
        void saveStuff() 
        {
            try
            {
                ...
                // Teleport to higher
                Server.tpToHigher = chkTpToHigher.Checked;
                ...
                GrpCommands.fillRanks();
                // Global Chat Options
                Server.useglobal = chkGlobalChat.Checked;
                Server.globalNick = txtGlobalNick.Text;
            }
            catch (Exception) { Server.s.Log("Failed to save properties."); }
        }

        public void SaveChatOptions()
        {
            // Anti-Spam Save
            Server.antiSpam = chkAntiSpam.Checked;
            Server.antiSpamOp = chkAntiSpamOp.Checked;
            int MsgsReq = Convert.ToInt16(txtMsgsRequired.Text);
            Server.spamCounter = MsgsReq;
            switch (cmbAntiSpamConsequence.Text.ToLower())
            {
                case "kick":
                    Server.antiSpamStyle = "Kick";
                    break;
                case "tempban":
                    Server.antiSpamStyle = "TempBan";
                    break;
                case "mute":
                    Server.antiSpamStyle = "Mute";
                    break;
                case "slap":
                    Server.antiSpamStyle = "Slap";
                    break;
                default:
                    Server.antiCapsStyle = "Kick";
                    break;
            }

            // Anti-Caps Load
            Server.antiCaps = chkAntiCaps.Checked;
            Server.antiCapsOp = chkAntiCapsOp.Checked;
            int CapsReq = Convert.ToInt16(txtCapsRequired.Text);
            Server.capsRequired = CapsReq;
            switch (cmbAntiCapsConsequence.Text.ToLower())
            {
                case "kick":
                    Server.antiCapsStyle = "Kick";
                    break;
                case "tempban":
                    Server.antiCapsStyle = "TempBan";
                    break;
                case "mute":
                    Server.antiCapsStyle = "Mute";
                    break;
                case "slap":
                    Server.antiCapsStyle = "Slap";
                    break;
                default:
                    Server.antiCapsStyle = "Kick";
                    break;
...
#region rankTab
        ...
        private void listRanks_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            txtReqHours.Text = foundRank.reqHours.ToString();
        }
        ...
        private void txtReqHours_TextChanged(object sender, EventArgs e)
        {
            if (txtReqHours.Text != "")
            {
                //int reqHours;
                //reqHours = int.Parse(txtReqHours.Text);
                if (txtReqHours.Text == "0") { txtReqHours.BackColor = SystemColors.Control; }
                else { txtReqHours.BackColor = Color.White; }
                int foundHours;
                try
                {
                    foundHours = int.Parse(txtReqHours.Text);
                }
                catch
                {
                    txtReqHours.Text = "0";
                ...
                //storedRanks[listRanks.SelectedIndex].reqHours = Convert.ToInt32(txtReqHours.Text);
                if (foundHours < 0) { txtReqHours.Text = "0"; return; }
                storedRanks[listRanks.SelectedIndex].reqHours = foundHours;
            }
        }
        ...
        private void btnAddRank_Click(object sender, EventArgs e)
        {
            ...
            Group newGroup = new Group((LevelPermission)5, 600, "CHANGEME", '1', "CHANGEME.txt", 0);
...
#region BlockTab
        ...
        private void btnDownloadMySQL_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Could not open download link!");
            }
        }

        private void chkTpToHigher_CheckedChanged(object sender, EventArgs e)
        {
            if (chkTpToHigher.Checked == true) { Server.tpToHigher = true; }
            else { Server.tpToHigher = false; }
        }

		private void btnPortCheck_Click(object sender, EventArgs e)
		{
			lblPortCheckResult.Text = "Checking...";
			lblPortCheckResult.BackColor = SystemColors.Control;
			...
			try
			{
				...
				if (resp == "ok")
				{
					lblPortCheckResult.Text = "Port Open!";
					lblPortCheckResult.BackColor = Color.LimeGreen;
					return;
				}
				if (resp == "failed")
				{
					lblPortCheckResult.Text = "Port Closed.";
					lblPortCheckResult.BackColor = Color.Red;
					return;
				}
			}
			catch (Exception ex)
			{
                Server.ErrorLog(ex);
				lblPortCheckResult.Text = "Error Checking.";
				lblPortCheckResult.BackColor = Color.Yellow;
				return;
			}
		}

        private void cmbGlobalColour_SelectedIndexChanged(object sender, EventArgs e)
        {
            lblGlobalColour.BackColor = Color.FromName(cmbGlobalColour.Items[cmbGlobalColour.SelectedIndex].ToString());
        }
        ...
        private void btnEditSwearWords_Click(object sender, EventArgs e)
        {
            eText = new EditText();
            eText.Show();
        }
        ...
        {
            Command.all.Find("text").Use(null, txtCreateNewText.Text);
        }

        private void tbThrottle_Scroll(object sender, EventArgs e)
        {
            Server.throttle = tbThrottle.Value;
        }
