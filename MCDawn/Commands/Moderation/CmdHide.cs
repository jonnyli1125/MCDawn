            ...
            try
            {
                ...
                if (message == "")
                {
                    ...
                    else
                    {
                        ...
                        Player.GlobalMessageOps("To Ops: " + p.color + p.name + Server.DefaultColor + " is now &fvisible" + Server.DefaultColor + ".");
                ...
                if (message.ToLower() == "s")
                {
                    ...
                    {
                        Player.GlobalDie(p, true);
                        Player.GlobalMessageAdmins("To Admins: " + p.color + p.name + Server.DefaultColor + " is now &finvisible" + Server.DefaultColor + ".");
                        Player.SendMessage(p, "You're now &finvisible&e.");
                    }
                    else
                    {
                        Player.GlobalSpawn(p, p.pos[0], p.pos[1], p.pos[2], p.rot[0], p.rot[1], false);
                        Player.GlobalMessageAdmins("To Admins: " + p.color + p.name + Server.DefaultColor + " is now &fvisible" + Server.DefaultColor + ".");
                        Player.SendMessage(p, "You're now &fvisible&e.");
                    }
                }
            }
