        ...
        public override void Use(Player p, string message)
        {
            ...
            if (who != null)
            {
                if (p != null)
                {
                    ...
                    if (p.group.Permission < LevelPermission.Builder && seconds > 120) { Player.SendMessage(p, Group.findPerm(LevelPermission.Guest).trueName + "s may only undo 2 minutes."); return; }
                    else if (p.group.Permission < LevelPermission.AdvBuilder && seconds > 300) { Player.SendMessage(p, Group.findPerm(LevelPermission.Builder).trueName + "s may only undo 300 seconds."); return; }
                    else if (p.group.Permission < LevelPermission.Operator && seconds > 1200) { Player.SendMessage(p, Group.findPerm(LevelPermission.AdvBuilder).trueName + "s may only undo 600 seconds."); return; }
            ...
            else
            {
                if (p != null)
                {
                    ...
                    if (seconds > 5400 && p.group.Permission < LevelPermission.Operator) { Player.SendMessage(p, "Only Operators may undo more than 90 minutes."); return; }
                }
