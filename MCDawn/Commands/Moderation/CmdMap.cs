...
namespace MCDawn
{
    public class CmdMap : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (p != null)
                if (p.group.Permission < LevelPermission.Operator && p.level.name.ToLower() != Server.HomePrefix.ToLower() + p.name.ToLower()) { Player.SendMessage(p, "Setting map options is reserved to OP+"); return; }
