...
namespace MCDawn
{
    public class CmdLike : Command
    {
        ...
        {
            if (!Server.useMySQL) { Player.SendMessage(p, "MySQL has not been configured! Please configure MySQL to use /like!"); return; }
        ...
        {
            Player.SendMessage(p, "/like [level] - Adds a like to [level]. If [level] is not given, the current level is used.");
        }
    }
}